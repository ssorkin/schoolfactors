<script>
  ...
  async function loadGroup(g) {
    ...
    rows = null;
    if (!cache.has(g)) {
      const raw = await (await fetch(`/data/groups/${g}.json`)).json();
      cache.set(
        g,
        raw.map((r) => ({
          ...r,
...
<svelte:head><title>Best for {LABELS[group]?.toLowerCase()} — SchoolFactors</title></svelte:head>
...
<div class="controls">
  <input
    class="filter"
    placeholder="Filter by name, district, or county…"
    bind:value={query}
