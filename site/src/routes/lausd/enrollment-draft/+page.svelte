<script>
  ...
  // ---- annual (ACS 1-year) reconciliation, 2010-2024, 2020 missing ----
  let annual = $derived(nar?.annual ?? null);
  let annBy = $derived((lvl) =>
    Object.fromEntries((annual?.[lvl] ?? []).map(([y, a, c]) => [y, { a, c }]))
  );
  let mA = $derived((y) => {
    const st = annBy('state')[y];
    return st ? (st.c - st.a) / st.c : null;
  });
  // net administrative import (negative = exporter), annual basis
  let annImport = $derived((lvl, y) => {
    const r = annBy(lvl)[y];
    const m = mA(y);
    return r && m != null ? r.c * (1 - m) - r.a : null;
  });
  let annSeries = $derived.by(() => {
    const out = [];
    for (let y = 2010; y <= 2024; y++) {
      const v = annImport('county', y);
      if (v != null) out.push({ y, v });
    }
    return out;
  });
  // consecutive runs so the missing 2020 renders as a genuine gap
  let annRuns = $derived.by(() => {
    const runs = [];
    let cur = [];
    for (const p of annSeries) {
      if (cur.length && p.y !== cur[cur.length - 1].y + 1) {
        runs.push(cur);
        cur = [];
      }
      cur.push(p);
  ...
  const LX = $derived((y) =>
    LM.left + ((y - 2010) / 14) * (LW - LM.left - LM.right));
  ...
  let aMax = $derived(Math.max(...annSeries.map((p) => -p.v), 1) * 1.08);
  const LYI = $derived(
    (v) => LM.top + ((3000 - v) / (3000 + aMax)) * (LH - LM.top - LM.bottom)
  );
  ...
  // ---- Conclusion: annual (ACS 1-year) accounting of where LAUSD-boundary
  //      children are counted, 2005-2024 (the 2020 ACS was never released).
  //      Cells arrive precomputed from the export (narrative.annual_cells). ----
  const CCATS = [
    { key: 'trad', label: 'LAUSD traditional', short: 'Traditional', color: '#eb6834' },
    { key: 'aff', label: 'LAUSD affiliated', short: 'Affiliated', color: '#f0b28a' },
    { key: 'ind', label: 'LAUSD independent', short: 'Independent', color: '#1e6b3a' },
    { key: 'phys_chr', label: 'Non-LAUSD charter in boundary', short: 'Non-LAUSD charter',
      color: '#1aa3a3' },
    { key: 'phys_oth', label: 'Other districts in person', short: 'Other district',
      color: '#2a78d6' },
    { key: 'vr', label: 'Virtual & out-of-county (est.)', short: 'Virtual+remote',
      color: '#4a3aa7' },
    { key: 'priv', label: 'Private school', short: 'Private', color: '#b5aea1' },
    { key: 'noten', label: 'Not enrolled', short: 'Not enrolled', color: '#d8d4cc' }
  ];
  // the two estimated cells carry ACS margins (est.moe_*); the rest are
  // CDE-exact or direct ACS categories
  const EST_MOE = { phys_oth: 'moe_phys_oth', vr: 'moe_vr' };
  let aRows = $derived(nar?.annual_cells ?? []);
  let aBy = $derived(Object.fromEntries(aRows.map((r) => [r.y, r])));
  let aCellOf = $derived((y, k) => aBy[y]?.cells?.[k]);
  // era mean over the years present (single 1-yr estimates are noisy; prose
  // anchors on short windows where a single year would mislead)
  let aMean = $derived((y0, y1, k) => {
    const vs = [];
    for (let y = y0; y <= y1; y++) if (aBy[y]) vs.push(aBy[y].cells[k]);
    return vs.length ? vs.reduce((s, v) => s + v, 0) / vs.length : null;
  });
  let aFirst = $derived(aRows.length ? aRows[0] : null);
  let aLast = $derived(aRows.length ? aRows[aRows.length - 1] : null);
  const AW = 900;
  const AH = 470;
  const AM = { top: 30, right: 214, bottom: 30, left: 214 };
  let aX0 = $derived(aRows.length ? aRows[0].y : 2005);
  let aX1 = $derived(aRows.length ? aRows[aRows.length - 1].y : 2024);
  const AX = $derived(
    (y) => AM.left + ((y - aX0) / Math.max(aX1 - aX0, 1)) * (AW - AM.left - AM.right)
  );
  let aMaxT = $derived(Math.max(...aRows.map((r) => r.total), 1) * 1.02);
  const AY = $derived((v) => AM.top + (1 - v / aMaxT) * (AH - AM.top - AM.bottom));
  // per-year cumulative edges, bottom-anchored: not-enrolled at the bottom,
  // traditional LAUSD on top — the falling top edge is the child population
  let aStack = $derived(
    aRows.map((r) => {
      let lo = 0;
      const edges = {};
      for (let k = CCATS.length - 1; k >= 0; k--) {
        ...
        edges[key] = { lo, hi: lo + r.cells[key] };
        lo += r.cells[key];
      }
      return { y: r.y, total: r.total, cells: r.cells, edges };
    })
  );
  // consecutive runs so the missing 2020 renders as a genuine gap
  let aSegs = $derived.by(() => {
    const runs = [];
    let cur = [];
    for (const p of aStack) {
      if (cur.length && p.y !== cur[cur.length - 1].y + 1) {
        runs.push(cur);
        cur = [];
      }
  ...
  const aBand = (run, key) =>
    run.map((p, i) => `${i ? 'L' : 'M'}${AX(p.y)},${AY(p.edges[key].hi)}`).join('') +
    [...run].reverse().map((p) => `L${AX(p.y)},${AY(p.edges[key].lo)}`).join('') +
    'Z';
  // end labels, greedily spaced downward so slivers (in-boundary charter is
  // ~1k in 2005) never collide with their neighbors
  const aLabels = (col) => {
    const out = CCATS.map((c) => ({
      ...c,
      v: col.cells[c.key],
      ly: (AY(col.edges[c.key].lo) + AY(col.edges[c.key].hi)) / 2 + 3.5
    }));
    for (let i = 1; i < out.length; i++) {
      out[i].ly = Math.max(out[i].ly, out[i - 1].ly + 13);
    }
    return out;
  };
  ...
  function aMove(e) {
    const rect = e.currentTarget.getBoundingClientRect();
    const px = ((e.clientX - rect.left) / rect.width) * AW;
    const py = ((e.clientY - rect.top) / rect.height) * AH;
    let best = null;
    for (const p of aStack) {
      if (!best || Math.abs(AX(p.y) - px) < Math.abs(AX(best.y) - px)) best = p;
    }
    if (!best) {
      aHover = null;
      return;
    ...
    for (const c of CCATS) {
      const ed = best.edges[c.key];
      if (py >= AY(ed.hi) && py <= AY(ed.lo)) {
        key = c.key;
        break;
      }
    }
...
<svelte:head>
  <title>LAUSD enrollment (revision draft) — SchoolFactors</title>
...
<p class="draftnote">
  Revision draft — incorporates ACS 1-year data through 2024. Will replace the
  live article on approval; the original is archived at
  <a href="/lausd/enrollment-v1">/lausd/enrollment-v1</a>.
</p>
<p class="byline">August 21, 2026 · revised August 22, 2026 · by Stephen Sorkin</p>
    ...
    <p class="caption">
      ...
      {:else}
        ...
        <span class="k"><span class="sw" style="background:#2a78d6"></span>Resident
          children 5–17 (ACS 5-yr; open dots = 1-yr estimates 2005–09, adjusted
          to 2010-census-consistent population controls)</span>
...
{#if stabAll.length > 1}
  ...
  <p class="method">
    ...
    same-window state offset, so the drift is absorbed. The same calibration
    holds on the annual (ACS 1-year) basis — 3–6.5%, trendless — which is what
    licenses the annual series in Chart 3.
  </p>
{/if}
...
<p>
  ...
  of county enrollment, past any earlier point in the data. The annual series
  (Chart 3) carries the ledger a year further: by calendar 2024 the county's
  deficit reaches roughly
  <b>{annImport('county', 2024) != null ? fmtN(-annImport('county', 2024)) : '…'}</b>
  children, and LAUSD's own net export a record
  <b>{annImport('lausd', 2024) != null ? fmtN(-annImport('lausd', 2024)) : '…'}</b>
  — {annImport('lausd', 2024) != null && annBy('lausd')[2024]
    ? fmtPct(-annImport('lausd', 2024) / annBy('lausd')[2024].c, 1)
    : '…'} of its adjusted enrollment. The children need not have moved.
  Increasingly, the administrative geography of their enrollment appears to
  have.
</p>

{#if annSeries.length > 5}
  <div class="chartwrap">
    <p class="title">Chart 3 · LA County net administrative import, per year
      (ACS 1-year estimates)</p>
    <svg viewBox="0 0 {LW} {LH}" role="img"
      aria-label="LA County net administrative import by ACS survey year"
      onmouseleave={() => (leakHover = null)}>
      {#each [0, -10000, -20000, -30000, -40000, -50000] as t}
        {#if -t < aMax}
      ...
      {/each}
      {#each annRuns as run}
        <polyline points={run.map((p) => `${LX(p.y)},${LYI(p.v)}`).join(' ')}
      ...
      {#each annSeries as p}
        ...
        {/if}
      {/each}
      <text x={LX(2020)} y={LYI(-2000)} text-anchor="middle" class="tick">no
        2020 ACS</text>
    </svg>
    <p class="caption">
      {#if leakHover != null}
        {@const p = annSeries.find((q) => q.y === leakHover)}
        <b>{p.y}</b>: net administrative import of {fmtN(p.v)} — roughly
        {fmtN(-p.v)} county-resident public-school children beyond the
        enrollment attributed to LA County schools that year
      {:else}
        The closed system, year by year: the county's balance sits within a
        fraction of a percent of zero from 2011 through 2018, then dives after
        2019 to a deficit of ~{fmtKk(-annSeries[annSeries.length - 1].v)}
        children by {annSeries[annSeries.length - 1].y} — beyond any earlier
        point in the annual record.
      {/if}
    </p>
    <p class="refrain">Apparent, net accounting — not measured student flows.
      Single-year ACS estimates; the 2020 ACS was never released.</p>
  </div>
...
<p>
  The whole story can be seen clearly in Chart 4 — the annual ACS record,
  {aFirst?.y ?? '…'} through {aLast?.y ?? '…'}, built so each band stays as
  close to measurement as the data allow. The three LAUSD bands and the
  in-boundary charter band are census-day counts; private school and "not
  enrolled" come straight from the ACS. The last two bands split the remainder
  with one allocation assumption: the county-wide pool of virtual and
  out-of-county enrollment is divided between boundary and rest-of-county
  children in proportion to resident public-school population. The boundary's
  share is the virtual-and-out-of-county band; "other districts in person"
  then follows from the rest of the county's own ledger — the seats its LEAs
  operate beyond flagged-virtual programs and in-boundary charters, less the
  rest-of-county children estimated to actually occupy classroom seats. Both
  carry the ACS margin of error (the ± in the table). Over the
  two decades the pie shrinks by
  {aFirst && aLast ? fmtKk(aFirst.total - aLast.total) : '…'} children — but
  market share also moves from LAUSD's own schools (traditional plus affiliated
  charter, {aFirst ? fmtKk(aFirst.cells.trad + aFirst.cells.aff) : '…'} →
  {aLast ? fmtKk(aLast.cells.trad + aLast.cells.aff) : '…'}) to alternatives.
  (1) Independent charters authorized by LAUSD add
  {aFirst && aLast ? fmtKk(aLast.cells.ind - aFirst.cells.ind) : '…'} students,
  with essentially all of the growth complete by 2019
  ({aCellOf(2019, 'ind') ? fmtKk(aCellOf(2019, 'ind')) : '…'} then, flat
  since). (2) Other in-person schools: non-LAUSD brick-and-mortar charters
  inside the boundary grow steadily but stay small
  ({aFirst ? fmtK1(aFirst.cells.phys_chr) : '…'} →
  {aLast ? fmtK1(aLast.cells.phys_chr) : '…'}), while the other-district
  estimate climbs from roughly
  {aMean(2010, 2012, 'phys_oth') != null ? fmtKk(aMean(2010, 2012, 'phys_oth')) : '…'}
  a year in the early 2010s to roughly
  {aMean(2013, 2018, 'phys_oth') != null ? fmtKk(aMean(2013, 2018, 'phys_oth')) : '…'}
  across 2013–2018 (single years are noisy; the margins run ±12–17k) — the
  hallmark of Act I. That move reads as a flight toward measured quality:
  districts scoring lower on both raw tests and the per-student Similar
  Student measure lost students to districts scoring higher on both. (3) After
  2019 the virtual-and-out-of-county band surges — from
  ~{aMean(2012, 2017, 'vr') != null ? fmtKk(aMean(2012, 2017, 'vr')) : '…'} a
  year across 2012–2017 to
  ~{aLast ? fmtKk(aLast.cells.vr) : '…'} by {aLast?.y ?? '…'} — while the
  other-district estimate eases back toward its early-2010s level
  (~{aMean(2019, 2024, 'phys_oth') != null ? fmtKk(aMean(2019, 2024, 'phys_oth')) : '…'}
  a year since 2019): Act II. We can
  only presume this is a flight of convenience rather than of measured
  quality: the virtual schools are largely not high-performing on either
  measure. Two notes on reading the chart. First, the 2005–2009 columns are
  corrected to 2010-census-consistent population controls: the estimates those
  ACS years actually weighted to overstated LA County's children by up to 5%,
  which uncorrected masquerades as tens of thousands of phantom out-of-county
  enrollment (the method note quantifies this). After correction the early
  virtual/remote estimate averages
  ~{aMean(2005, 2009, 'vr') != null ? fmtKk(aMean(2005, 2009, 'vr')) : '…'} —
  storefront independent-study programs and the first virtual academies were
  already operating — but it sits close to its own margin of error, so we
  read no more into it. Second, the pandemic shows up in the small bands: in
  2021 private school jumped to
  {aCellOf(2021, 'priv') ? fmtKk(aCellOf(2021, 'priv')) : '…'} and "not
  enrolled" to {aCellOf(2021, 'noten') ? fmtKk(aCellOf(2021, 'noten')) : '…'},
  both easing since.
</p>
...
{#if aStack.length > 2}
  <div class="chartwrap">
    <p class="title">Chart 4 · Where the LAUSD attendance area's children are
      counted, per year (ACS 1-year estimates, {aX0}–{aX1})</p>
    <svg class="wide" viewBox="0 0 {AW} {AH}" role="img"
      aria-label="Where LAUSD-area resident children are counted, annual series"
      onmousemove={aMove} onmouseleave={() => (aHover = null)}>
      {#each aSegs as run}
        {#each CCATS as cat}
          <path d={aBand(run, cat.key)} fill={cat.color}
            fill-opacity={aHover?.key === cat.key ? 1 : 0.85}
            stroke="#fcfcfb" stroke-width="1" />
        {/each}
      {/each}
      {#if aBy[2019] && aBy[2021]}
        <text x={AX(2020)} y={AY(aBy[2019].total) - 14} text-anchor="middle"
          class="tick">no 2020 ACS</text>
      {/if}
      {#if aHover && aBy[aHover.y]}
        <line x1={AX(aHover.y)} y1={AY(aBy[aHover.y].total)} x2={AX(aHover.y)}
          y2={AH - AM.bottom} stroke="#52514e" stroke-dasharray="3 3" />
      {/if}
      {#each aStack as p}
        {#if p.y % 4 === 1 || p.y === aX1}
          <text x={AX(p.y)} y={AH - 8} text-anchor="middle" class="tick">{p.y}</text>
        {/if}
      {/each}
      {#if aStack.length}
        {#each aLabels(aStack[0]) as l}
          <text x={AM.left - 7} y={l.ly} text-anchor="end" class="nlabel">
            <tspan class="nname">{l.label}</tspan>
            <tspan class="nval">&#8201;{fmtKk(l.v)}</tspan></text>
        {/each}
        {#each aLabels(aStack[aStack.length - 1]) as l}
          <text x={AW - AM.right + 7} y={l.ly} class="nlabel">
            <tspan class="nname">{l.label}</tspan>
            <tspan class="nval">&#8201;{fmtKk(l.v)}</tspan></text>
        {/each}
        <text x={AM.left} y={AY(aStack[0].total) - 8} text-anchor="middle"
          class="tick">{fmtKk(aStack[0].total)}</text>
        <text x={AW - AM.right} y={AY(aStack[aStack.length - 1].total) - 8}
          text-anchor="middle" class="tick">
          {fmtKk(aStack[aStack.length - 1].total)}</text>
      {/if}
    </svg>
    <p class="caption">
      {#if aHover && aBy[aHover.y]}
        {@const r = aBy[aHover.y]}
        <b>{aHover.y}</b>:
        {#each CCATS as cat}
          <span class="k" class:hovered={aHover.key === cat.key}>
            <span class="sw" style="background:{cat.color}"></span>{cat.short}
            {fmtN(r.cells[cat.key])}{#if EST_MOE[cat.key] && r.est}<span
              class="moe">±{fmtKk(r.est[EST_MOE[cat.key]])}</span>{/if}</span>
        {/each}
        · total {fmtN(r.total)}
      {:else}
        Hover for any year's full accounting. Bands are bottom-anchored, so the
        falling top edge is the shrinking child population; the break at 2020
        is the unreleased ACS.
      {/if}
    </p>
    <p class="refrain">Apparent, net accounting — not measured student flows.
      Single-year ACS estimates; year-to-year wiggles are partly sampling
      noise.</p>
  </div>
  ...
  <div class="tablewrap">
    <table>
      <thead>
        <tr><th>ACS year</th>
          {#each CCATS as cat}
            <th><span class="typedot" style="background:{cat.color}"></span>{cat.short}</th>
          {/each}
          <th>= resident&nbsp;5–17</th></tr>
      </thead>
      <tbody>
        {#each aRows as r, i}
          {#if i && r.y !== aRows[i - 1].y + 1}
            <tr class="gaprow">
              <td colspan={CCATS.length + 2}>2020 · no ACS released</td>
            </tr>
          {/if}
          <tr>
            <td>{r.y}</td>
            {#each CCATS as cat}
              <td
                title={cat.key === 'phys_oth' && r.est
                  ? `rest-of-county net ${fmtN(r.est.rest_net)} + their share ` +
                    `of the virtual/out-of-county pool ${fmtN(r.est.rest_pool)}`
                  : undefined}>
                {fmtN(r.cells[cat.key])}{#if EST_MOE[cat.key] && r.est}<span
                  class="moe"> ±{fmtKk(r.est[EST_MOE[cat.key]])}</span>{/if}
              </td>
            {/each}
            <td class="sumcell">{fmtN(r.total)}</td>
          </tr>
        {/each}
      </tbody>
    </table>
  </div>
  <h3>Through 2024: the shift accelerates</h3>
  <p>
    Chart 4's record now runs through calendar {aX1}, and every line of it
    points the same way. LAUSD's apparent net export reached
    {annImport('lausd', 2024) != null ? fmtN(-annImport('lausd', 2024)) : '…'}
    children in 2024 — its largest on record — while LA County's deficit held
    near {annImport('county', 2024) != null ? fmtKk(-annImport('county', 2024)) : '…'}.
    On the enrollment side, traditional LAUSD schools fell another
    {clsYearBy[2026] && clsYearBy[2025]
      ? fmtPct(clsYearBy[2026].trad / clsYearBy[2025].trad - 1, 1)
      : '…'} in 2025–26 — the steepest single-year drop in the series — while
    LAUSD-authorized independent charters drifted slightly down and statewide
    flagged-virtual enrollment grew to
    {virtual.length ? fmtKk(virtual[virtual.length - 1][1]) : '…'}. Act II is
    not a closed chapter; on the most recent data it is still gathering speed.
  </p>
  <p class="method">
    Assumptions, in the open. (1) Columns are ACS 1-year survey years; each
    CDE-side series is the mean of the two census days the survey year
    straddles, and seats are converted to resident children with the same-year
    statewide Census/CDE adjustment; in-boundary seats (LAUSD's three classes
    and the non-LAUSD brick-and-mortar charter campuses located inside the
    boundary, virtual-flagged campuses excluded) are treated as boundary
    residents on net. (2) The last two bands rest on an exact identity plus
    one allocation assumption. The identity: the boundary's out-of-LAUSD
    children equal the rest of the county's net absorption plus the
    county-wide pool of virtual and out-of-county enrollment. The assumption:
    that pool draws on boundary and rest-of-county children in proportion to
    resident public-school population (we know of no better basis). The
    boundary's share is the "virtual &amp; out-of-county" band — it asserts
    where enrollment is <em>not</em>: not in a classroom in LA County. The
    rest-of-county share frees seats in their districts, so it credits the
    other-district band: rest-of-county LEAs' adjusted seats, less
    flagged-virtual programs and in-boundary charters, less the rest-of-county
    children estimated to remain in classrooms (hover a table cell for the two
    components). Without that credit the rest-of-county net alone turns
    negative after 2019 — their children also leave for virtual — which would
    wrongly zero the permit channel. The virtual flag is a lower bound on
    nonclassroom-based enrollment, and Act II ties the post-2019 out-of-county
    rise to virtual/independent-study counties. (3) The two estimated bands
    carry ±90% ACS margins combined root-sum-square from the published cell
    margins (approximate — covariance from population controls is ignored). (4) The 2005–2009 ACS levels are
    corrected to the Census Bureau's 2010-census-consistent intercensal
    estimates: each of those survey years was weighted to its own vintage of
    Census-2000-based county estimates, which overstated LA County children by
    5.1% (2005) shrinking to 0% (2009–10) — visible as the county's ACS share
    of the state's public-school children running up to 1.4 points above its
    CDE enrollment share before 2010, then within ±0.1 points every year
    2011–2018. The correction rescales county and boundary levels by the
    intercensal-to-vintage ratio (state levels likewise for the calibration);
    it is an explicit, documented transform, not a fit to the enrollment data.
    (5) Single-year estimates are noisier than the old 5-year windows, pre-2010
    samples more so; the 2020 ACS was never released, so that year is a
    genuine gap. Apparent, net accounting — not measured student flows.
  </p>
{/if}


<p class="method">
  Method caveats: annual figures use ACS 1-year estimates (published only for
  geographies of 65,000+ residents, so the per-district tables stay on 5-year
  windows); an ACS survey year straddles two school years, so its CDE
  counterpart is the mean of the two adjacent census days; the 2020 ACS was
  never released; pre-2010 1-year samples were smaller and noisier. Every
  cross-boundary number on this page is an apparent,
  ...
  resident points are single-year estimates — adjusted to 2010-census-consistent
  intercensal population controls, since the estimates those years originally
  weighted to overstated LA County children by up to 5% — and noisier than the
  5-year windows. The virtual-school flag is present-day, so early years of that series
  are approximate.
</p>
...
<p class="endnote">
  <b>Revision note.</b> This article was revised on August 22, 2026 to
  incorporate ACS 1-year estimates through calendar 2024: the annual county
  series in Chart 3, the "Through 2024" postscript, and Chart 4's accounting,
  which moved from four 5-year windows to the full annual record (2005–2024)
  and was rebuilt bottom-up: exact census-day counts for the LAUSD and
  in-boundary charter bands, and the remainder split between other districts
  (a rest-of-county ledger estimate) and virtual/out-of-county enrollment by
  allocating the county-wide virtual/remote pool in proportion to resident
  population, with ACS margins of error on both. ACS levels for 2005–2009 are
  corrected to
  2010-census-consistent intercensal population controls — the estimates those
  years originally weighted to overstated LA County children by up to 5%,
  which otherwise masquerades as phantom cross-county enrollment. The original
  August 21, 2026 version, based entirely on 5-year windows ending 2023, is
  archived at <a href="/lausd/enrollment-v1">/lausd/enrollment-v1</a>.
</p>
